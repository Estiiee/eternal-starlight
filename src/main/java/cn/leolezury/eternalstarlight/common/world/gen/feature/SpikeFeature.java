...
public class SpikeFeature extends ESFeature<SpikeFeature.Configuration> {
	...
	public boolean place(FeaturePlaceContext<SpikeFeature.Configuration> context) {
		...
		ValueMapGenerator.place(new MergedProvider(entries), (pos, value) -> setBlockIfEmpty(level, pos.offset(origin), config.spike().getState(random, pos.offset(origin)), true, s ->
			s.is(ESTags.Blocks.BASE_STONE_STARLIGHT) || s.canBeReplaced()));
		return true;
	}
