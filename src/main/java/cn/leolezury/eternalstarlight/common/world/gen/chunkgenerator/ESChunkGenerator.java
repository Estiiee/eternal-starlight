...
public class ESChunkGenerator extends NoiseBasedChunkGenerator {
	...
	public ChunkAccess doFill(Blender blender, StructureManager structureManager, RandomState randomState, ChunkAccess chunkAccess, int i, int j) {
		...
		if (!adaptiveStructures.isEmpty()) {
			for (StructureStart start : structureManager.startsForStructure(chunkPos, adaptiveStructures::contains)) {
	...
	private static Pair<BlockPos, Holder<Structure>> checkStructureAtLandmark(
		...
		for (Holder<Structure> structureHolder : structureHolders) {
			StructureCheckResult checkResult = structureManager.checkStructurePresence(chunkPos, structureHolder.value(), skipKnownStructures);
