...
public record ExplosionShockParticleOptions(Vector3f fromColor, Vector3f toColor, float lengthScale, float width, float lifeScale) implements ParticleOptions {
	...
	public static final ExplosionShockParticleOptions FLARE_LONG = fromIntColor(new Vector3f(222, 112, 255), new Vector3f(255, 255, 116), 4, 0.06f, 1);
	...
	public static ExplosionShockParticleOptions fromIntColor(Vector3f fromColor, Vector3f toColor, float lengthScale, float width, float lifeScale) {
		return new ExplosionShockParticleOptions(
	...
	public static final MapCodec<ExplosionShockParticleOptions> CODEC =
	...
	public static final Deserializer<ExplosionShockParticleOptions> DESERIALIZER =
		new Deserializer<>() {

			@Override
			public ExplosionShockParticleOptions fromCommand(ParticleType<ExplosionShockParticleOptions> type, StringReader reader) {
				return new ExplosionShockParticleOptions(
					new Vector3f(1, 1, 1),
					new Vector3f(1, 1, 1),
					1F,
			...
			@Override
			public ExplosionShockParticleOptions fromNetwork(ParticleType<ExplosionShockParticleOptions> type, FriendlyByteBuf buf) {
				return new ExplosionShockParticleOptions(
					buf.readVector3f(),
					buf.readVector3f(),
					buf.readFloat(),
