...
public record RippleParticleOptions(ParticleType<RippleParticleOptions> type, SmoothSegmentedValue radius, SmoothSegmentedValue width, Vector3f color, int lifetime) implements ParticleOptions {
	...
	public static MapCodec<RippleParticleOptions> codec(ParticleType<RippleParticleOptions> type) {
		return RecordCodecBuilder.mapCodec(instance -> instance.group(
			...
			(radius, width, color, lifetime) ->
				new RippleParticleOptions(type, radius, width, color, lifetime)
		));
	}

	public static final ParticleOptions.Deserializer<RippleParticleOptions> DESERIALIZER =
		new ParticleOptions.Deserializer<>() {
			@Override
			public RippleParticleOptions fromCommand(
				ParticleType<RippleParticleOptions> type,
				StringReader reader
			) throws CommandSyntaxException
			{
				throw new CommandSyntaxException(
					CommandSyntaxException.BUILT_IN_EXCEPTIONS.dispatcherUnknownArgument(),
					new LiteralMessage("Ripple particle options cannot be specified from commands")
				);
			}

			@Override
			public RippleParticleOptions fromNetwork(
				ParticleType<RippleParticleOptions> type,
				FriendlyByteBuf buf
			) {
				return RippleParticleOptions.readFromNetwork(type, buf);
			}
		};

	public static RippleParticleOptions readFromNetwork(
		ParticleType<RippleParticleOptions> type,
		FriendlyByteBuf buf
	) {
		SmoothSegmentedValue radius = SmoothSegmentedValue.readFromNetwork(buf);
		SmoothSegmentedValue width = SmoothSegmentedValue.readFromNetwork(buf);
		...
		return new RippleParticleOptions(type, radius, width, color, lifetime);
	}

	public void writeToNetwork(FriendlyByteBuf buf) {
		radius.writeToNetwork(buf);
		width.writeToNetwork(buf);
