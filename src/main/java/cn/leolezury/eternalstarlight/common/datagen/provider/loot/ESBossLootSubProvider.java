...
public class ESBossLootSubProvider implements LootTableSubProvider {
	...
	public void generate(BiConsumer<ResourceLocation, LootTable.Builder> consumer) {
		...
		consumer.accept(ESLootTables.BOSS_PERMAFROST,
			LootTable.lootTable()
				.withPool(LootPool.lootPool()
					.add(LootTableReference.lootTableReference(ESLootTables.BOSS_COMMON)))
				.withPool(LootPool.lootPool()
				...
				.withPool(LootPool.lootPool()
					...
					.add(LootItem.lootTableItem(ESItems.STARLIT_PAINTING.get()).apply(SetNbtFunction.setTag(makePaintingTag(ESPaintingVariants.ABSOLUTE_ZERO))))));
		...
		consumer.accept(ESLootTables.BOSS_STARLIGHT_GOLEM,
			LootTable.lootTable()
				...
				.withPool(LootPool.lootPool()
					.when(LootItemRandomChanceCondition.randomChance(0.5f))
					.add(LootItem.lootTableItem(ESItems.STARLIT_PAINTING.get()).apply(SetNbtFunction.setTag(makePaintingTag(ESPaintingVariants.ENERGIZED))))));
		...
		consumer.accept(ESLootTables.BOSS_LUNAR_MONSTROSITY,
			LootTable.lootTable()
				...
				.withPool(LootPool.lootPool()
					.when(LootItemRandomChanceCondition.randomChance(0.5f))
					.add(LootItem.lootTableItem(ESItems.STARLIT_PAINTING.get()).apply(SetNbtFunction.setTag(makePaintingTag(ESPaintingVariants.MONSTROUS))))));
	}

	private static CompoundTag makePaintingTag(ResourceKey<PaintingVariant> variantKey) {
		CompoundTag tag = new CompoundTag();
		tag.putString("id", EternalStarlight.ID + ":painting");
		tag.putString("variant", variantKey.location().toString());
		return tag;
	}
}
